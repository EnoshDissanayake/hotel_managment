...
AddOtherItem aoi;
   // Invoice i;
    ...
    public void loadStock() {
        try {
            ResultSet rs = MySQL.search("SELECT DISTINCT `stock`.`id`,`product`.`id` AS `product_id`,`category`.`name` AS `category`,`brand`.`name` AS `brand`,`product`.`name`AS `product`,`stock`.`qty`,`grn_item`.`buying_price`,`stock`.`selling_price`,`stock`.`mfd`,`stock`.`exd` FROM `stock` INNER JOIN `grn_item` ON `grn_item`.`stock_id`=`stock`.`id` INNER JOIN `product` ON `stock`.`product_id`=`product`.`id` INNER JOIN `brand` ON `product`.`brand_id` = `brand`.`id` INNER JOIN `category` ON \n"
                    + "`product`.`category_id`= `category`.`id`ORDER BY `product`.`name` ASC ; ");
    ...
    public void searchStock() {
       try {
            ...
            if (!sp_min.isEmpty()) {
                ...
                if (sp_max.isEmpty()) {
                    //min notempty/max empty
                    queryVector.add("`stock`.`selling_price`>='" + sp_min + "'");
                } else {
                    //min notempty/max not empty
            ...
            if (mfd_fr != null) {
                if (mfd_to == null) {
                    ...
                    //min notempty/max not empty
                    queryVector.add("`stock`.`mfd`>='" + mfd_fr + "' AND `stock`.`selling_price`<='" + mfd_to + "' ");
                }
            ...
            ResultSet rs = MySQL.search("SELECT DISTINCT `stock`.`id`,`product`.`id`,`category`.`name`,`brand`.`name`,`product`.`name`,`stock`.`quantity`,`grn_item`.`buying_price`,`stock`.`selling_price`,`stock`.`mfd`,`stock`.`exd` FROM `stock` INNER JOIN `grn_item` ON `grn_item`.`stock_id`=`stock`.`id` INNER JOIN `product` ON `stock`.`product_id`=`product`.`id` INNER JOIN `brand` ON `product`.`brand_id` = `brand`.`id` INNER JOIN `category` ON "
                    + "`product`.`category_id`= `category`.`id`" + wherequery + "ORDER BY " + sortquery + " ");
            DefaultTableModel dtm = (DefaultTableModel) jTable1.getModel();
            dtm.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
                            ...
                            .addGroup(jPanel2Layout.createSequentialGroup()
                                ...
                                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(jPanel2Layout.createSequentialGroup()
                                        ...
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(jDateChooser4, javax.swing.GroupLayout.DEFAULT_SIZE, 72, Short.MAX_VALUE))
                                    .addGroup(jPanel2Layout.createSequentialGroup()
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        ...
        if (evt.getClickCount() == 2) {
            aoi.jLabel12.setText(jTable1.getValueAt(selectedRow, 0).toString());
            aoi.jLabel30.setText(jTable1.getValueAt(selectedRow, 1).toString());
            aoi.jLabel12.setText(jTable1.getValueAt(selectedRow, 4).toString());
            aoi.jLabel18.setText(jTable1.getValueAt(selectedRow, 3).toString());
            aoi.jLabel20.setText(jTable1.getValueAt(selectedRow, 2).toString());
            aoi.jLabel32.setText(jTable1.getValueAt(selectedRow, 8).toString());
            aoi.jLabel28.setText(jTable1.getValueAt(selectedRow, 7).toString());
            aoi.jLabel31.setText(jTable1.getValueAt(selectedRow, 9).toString());
        }
        // Set to invoice
